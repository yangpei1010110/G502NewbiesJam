...
public class NoesisFontEditor: Editor
{
    ...
    public void OnEnable()
    {
        ...
        {
            _dummyMesh = new Mesh();
            _dummyMesh.vertices = new Vector3[3] { new Vector3(0, 0, 0), new Vector3(1, 0, 0), new Vector3(0, 1, 0) };
    ...
    private RenderTexture RenderPreview(Noesis.View view, int width, int height)
    {
        try
        {
            if (CanRender() && view != null && view.Content != null)
            {
                ...
                // Workaround to avoid Unity optimization. If we don't do this, our Metal renderer will receive a null CurrentCommandEncoder
                _commands.DrawMesh(_dummyMesh, new Matrix4x4(), new Material(Shader.Find("UI/Default")), 0, -1);
